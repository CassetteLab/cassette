...
struct SettingsView: View {
    ...
    private func seedPreviousMonth() async {
        ...
        // Require real server tracks — abort rather than pollute with synthetic stubs
        let songs: [Song]
        do {
            let fetched = try await container.libraryService.randomSongs(size: 5)
            guard !fetched.isEmpty else {
                Logger.stats.error("[WRAPPED-SEED] FAILED — server returned 0 tracks, seed aborted")
                return
            }
            songs = fetched
        } catch {
            Logger.stats.error("[WRAPPED-SEED] FAILED — no real tracks available (\(error, privacy: .public)), seed aborted")
            return
        }
